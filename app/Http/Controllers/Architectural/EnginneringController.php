...
class EnginneringController extends WebController
{
    ...
    public function enginStart(Request $request, $id = 0)
    {
        ...
        $data['userList'] = DB::table('users')->where('status', 1)
            ->where('department_id',6)
            ->orderby('name')->select(['id', 'name', 'department_id'])->get();
    ...
    //查看洽谈工程设计详情
    public function enginStartDetail(Request $request, $id, $programme_id)
    {
        $this->user();
        //项目子工程
        $engineering = DB::table('engineering')->where('id', $id)->first();
        ...
        return $this->enginDetail($engineering, $project, $id, $programme_id);
    }

    //查看实施工程设计详情
    public function enginConductDetail(Request $request, $id, $programme_id)
    {
        $this->user();
        //项目子工程
        $engineering = DB::table('engineering')->where('id', $id)->first();
    ...
    //查看竣工工程设计信息
    public function enginCompletedDetail(Request $request, $id, $programme_id)
    {
        $this->user();
        //项目子工程
        $engineering = DB::table('engineering')->where('id', $id)->first();
    ...
    //查看终止项目工程设计信息
    public function enginTerminationDetail(Request $request, $id, $programme_id)
    {
        $this->user();
        //项目子工程
        $engineering = DB::table('engineering')->where('id', $id)->first();
    ...
    //查询详情
    protected function enginDetail($engineering, $project, $id, $programme_id)
    {
        $data['navid'] = 35;
        $data['subnavid'] = 3500;

        $programme = DB::table('engin_programme')
            ->where('engin_id', $id)
            ->where('id', $programme_id)
            ->first();
        if (empty($programme)) {
            //设计人员可以操作更改工程设计详情
            return redirect('/architectural/enginStart/' . $engineering->project_id . '?status=2&notice=' . '工程方案不存在');
        }
