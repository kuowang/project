            ...
            <div class="span12">
                <div class="widget">
                    <div class="widget-header" style="text-align: center">
                        <div class="title">
                            <span class="btn btn-info">施工进度管理</span>
                        </div>
                        @if(((in_array(30010301,$pageauth) && $engineering->progress_uid == $uid ) || in_array(30010301,$manageauth)))
                            @if($engineering->status ==1) <!-- 仅施工工程才能编辑-->
                            <div class="title" style="float: right;">
                                <a href="/progress/editProgressActualManage/{{$engin_id}}">
                        ...
                        <div id="dt_example" class="example_alt_pagination">
                            ...
                            <table class="layui-table layui-form">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        {{isset($progress_info->progress_actual_all_start_time)?$progress_info->progress_actual_all_start_time:''}}
                                    </td>
                                    <td>
                                        {{isset($progress_info->progress_actual_all_end_time)?$progress_info->progress_actual_all_end_time:''}}
                                    </td>
                                    <td>
                                        {{isset($progress_info->progress_actual_all_duration)?$progress_info->progress_actual_all_duration:''}}
                                    </td>
                                </tr>
                                <tr>
                                    <td>各系统工程施工周期</td>
                                ...
                                @if(!empty($engin_arch))
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            {{isset($progress_duration[$v->sub_arch_id]->progress_actual_start_time)?$progress_duration[$v->sub_arch_id]->progress_actual_start_time:''}}
                                        </td>
                                        <td>
                                            {{isset($progress_duration[$v->sub_arch_id]->progress_actual_end_time)?$progress_duration[$v->sub_arch_id]->progress_actual_end_time:''}}
                                        </td>
                                        <td>
                                            {{isset($progress_duration[$v->sub_arch_id]->progress_actual_duration)?$progress_duration[$v->sub_arch_id]->progress_actual_duration:''}}
                                        </td>
                                    </tr>
                                    @endforeach
                                @endif
                                                                    ...
                                                                    @if($arch['is_synchro'] ==1)
                                                                        ...
                                                                        <div class="progresszhouqi7" style="">
                                                                            @if(isset($progress_process[$arch['param_id']]))
                                                                                <input type="text" is_synchro="1"disabled="disabled"  id="param_id_{{$arch['param_id']}}" class="progresszhouqi8" value="{{$progress_process[$arch['param_id']]}}" name="arch_duration_actual[{{$arch['param_id']}}]" style="padding:4px 0;" onclick="key(this)" onchange='selectzhouqi("{{$v->sub_arch_id}}","{{$k}}",this)'>
                                                                            @else
                                                                                <input type="text" is_synchro="1"disabled="disabled"  id="param_id_{{$arch['param_id']}}"  class="progresszhouqi8" name="arch_duration_actual[{{$arch['param_id']}}]" style="padding:4px 0;"  onclick="key(this)" onchange='selectzhouqi("{{$v->sub_arch_id}}","{{$k}}",this)'>
                                                                            @endif
                                                                        </div>
                                                                    @else
                                                                        ...
                                                                        <div class="progresszhouqi7" style="">
                                                                            ...
                                                                            @else
                                                                                <input type="text" is_synchro="2"disabled="disabled"  id="param_id_{{$arch['param_id']}}"  class="progresszhouqi8"  name="arch_duration_actual[{{$arch['param_id']}}]" style="padding:4px 0;color:red" onclick="key(this)" onchange='selectzhouqi("{{$v->sub_arch_id}}","{{$k}}",this)'>
                                                                            @endif
                                                                        </div>
                                                                    @endif
                            ...
                            <table class="layui-table layui-form">
                                ...
                                <tr>
                                    ...
                                    <td>{{isset($progress_info->progress_actual_all_period )?$progress_info->progress_actual_all_period :''}}</td>
                                    <td class="pro-title">流程周期(天)</td>
                                    <td>{{isset($progress_info->progress_actual_period )?$progress_info->progress_actual_period :''}}</td>
                                    <td class="pro-title">同步周期(天)</td>
                                    <td>{{isset($progress_info->progress_actual_synchronization_period )?$progress_info->progress_actual_synchronization_period :''}}</td>
                                    <td class="pro-title">施工总工时(天)</td>
                                    <td>{{isset($progress_info->progress_actual_work_hours )?$progress_info->progress_actual_work_hours :''}}</td>
                                </tr>
                                </tbody>
                            </table>
